...
class ObservationCollector{
 ...
 async aviation(){const chunks=[];for(let i=0;i<this.ids.length;i+=5)chunks.push(this.ids.slice(i,i+5));
  await mapLimit(chunks,4,ids=>this.task('AWC:'+ids.join(','),async()=>{
   ...
   return {stations:ids,records:n,recordIds,receivedAt:r.receivedAt,url:r.url,httpDate:r.httpDate,httpAge:r.httpAge};
  }));
 }
 async nws(){await mapLimit(this.ids,4,id=>this.task('NWS:'+id,async()=>{
  ...
  let n=0;for(const f of r.data.features){const o=C.fromNWS(f.properties,id,C.ms(r.receivedAt),r.url,this.ids);if(o){this.add(o);n++;}}
  return {stations:[id],records:n,receivedAt:r.receivedAt,url:r.url,httpDate:r.httpDate,httpAge:r.httpAge};
 }));}
 ...
 publishShared(){const now=this.clock();const selected=C.selectRows(Object.values(this.state.variants),now);
  atomic(path.join(this.out,'shared.json'),{schemaVersion:1,version:'1.0.0',generatedAt:C.iso(now),rows:selected,
   ...
   note:'DSM and nearby estimates excluded. Timestamp validation required by each consumer.'});
 }
 publishDashboard(){const now=this.clock(),rows=Object.values(this.state.variants);W.gradeArrivals(this.state,rows,this.cfg,now,(k,d)=>this.log(k,d));
  ...
  const stations=Object.entries(this.cfg.targets).map(([id,s])=>{
   const own=selected.filter(r=>r.station===id&&!r.conflict&&r.f!=null),last=own.at(-1);
   ...
   for(const r of selected.filter(x=>x.station===id&&!x.conflict))for(const e of Shadow.parseMetar({icaoId:r.station,obsTime:C.ms(r.t)/1000,rawOb:r.raw},C.ms(r.firstReceivedAt)))evidence[e.key]={...e,sourceUrl:r.sourceUrl,receivedAt:r.firstReceivedAt,firstSeenAt:r.firstReceivedAt};
   ...
   return {station:id,name:s.name,date:curDate,observed:last?{f:last.f,precisionC:last.precisionC,at:last.t,receivedAt:last.firstReceivedAt,source:last.source,sourceUrl:last.sourceUrl,
    ...
    evidence:official,neighbor:nc,checks,dsm:this.state.dsm[id+'|'+curDate]||null,
  ...
  const snap={schemaVersion:1,version:'1.0.0',generatedAt:C.iso(now),paperOnly:true,stations,sampling:this.state.sampling,
